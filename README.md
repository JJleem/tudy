...
> README의 이미지가 표시되려면 아래 경로에 스크린샷을 저장해 주세요.

```
docs/screenshots/
├── home.png        # 홈화면 (로고 + 과목 카드)
├── chat.png        # 대화 화면 (초록/빨강 블록 + 점수 박스 보이는 상태)
├── graph.png       # 개념 그래프 SVG
├── summary.png     # 세션 요약 모달
└── progress.png    # 과목 선택 화면 (진도 배지)
```
